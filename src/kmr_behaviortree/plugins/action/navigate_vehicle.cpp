...
public:
  ...
  void on_tick() override
  {
    ...
    auto goal_opt = config().blackboard->get<geometry_msgs::msg::PoseStamped>("current_goalpose");
    
    // Check if the goal is valid by checking if the frame_id is empty (or implement more appropriate check)
    if (goal_opt.header.frame_id.empty())
    {
      RCLCPP_ERROR(node_->get_logger(), "NavigateVehicleAction: current_goalpose not found in blackboard");
      return;
    }
    
    current_goalpose = goal_opt;  // No dereferencing needed
