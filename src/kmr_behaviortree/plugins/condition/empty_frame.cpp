...
public:
  ...
  isFrameEmpty()
  {
    ...
    // Check for input and throw if missing (ROS 2 Humble update)
    if (!getInput("check_frame", check_frame))
    {
        throw BT::RuntimeError("Missing parameter [check_frame] in EmptyFrameCondition");
    }

    bool bb_frame = config().blackboard->get<bool>(check_frame);
    if (bb_frame) {
