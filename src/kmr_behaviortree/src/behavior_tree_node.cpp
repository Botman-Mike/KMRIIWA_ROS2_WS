...
#include "tf2_geometry_msgs/tf2_geometry_msgs.hpp"  // Changed from .h to .hpp
...
: Node("behavior_tree_node")
{
  ...
  // Fix parameter declarations by providing default values
  declare_parameter("plugin_lib_names", plugin_libs);
  declare_parameter("bt_xml_filename", "");  // Add default empty string
  declare_parameter("WS1.position", "0.0,0.0");  // Add default position
  declare_parameter("WS1.orientation", 0.0);  // Add default orientation
  declare_parameter("WS2.position", "0.0,0.0");
  declare_parameter("WS2.orientation", 0.0);
  declare_parameter("WS3.position", "0.0,0.0");
  declare_parameter("WS3.orientation", 0.0);
  declare_parameter("HOME.position", "0.0,0.0");
  declare_parameter("HOME.orientation", 0.0);
  declare_parameter("goal_list", std::vector<std::string>{});  // Default empty vector
...
private:
  ...
  bool send_navigation_goal(geometry_msgs::msg::PoseStamped pose){
    ...
    auto is_action_server_ready = action_client_->wait_for_action_server(std::chrono::seconds(5));
      ...
      if (rclcpp::spin_until_future_complete(client_node_, future_goal_handle) != rclcpp::FutureReturnCode::SUCCESS)
      {
      ...
      if (rclcpp::spin_until_future_complete(client_node_, result_future) != rclcpp::FutureReturnCode::SUCCESS){
