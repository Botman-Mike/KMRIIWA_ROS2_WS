...
def generate_launch_description(argv=sys.argv[1:]):
    ...
    return LaunchDescription([
        ...
        launch_ros.actions.Node(
            ...
            name="kmp_laserscan_node",          # Changed from node_name
            output="screen",
            emulate_tty=True,
        ...
        launch_ros.actions.Node(
            package="kmr_communication",
            executable="kmp_odometry_node.py",   # Changed from node_executable
            name="kmp_odometry_node",           # Changed from node_name
            output="screen",
            emulate_tty=True,
            arguments=['-c', connection_type_UDP,'-ro',robot]),
    ])
